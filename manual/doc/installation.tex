...
\ctg requires the following system software:
...
\ctg requires the following scientific software:
\begin{description}[labelwidth=.15\textwidth]
...
To use the ctg third-party repository, create a new file \filename{ctg.list} within
...
You can then proceed with the ctg installation as documented in \refsec{inst:ctg}.
...
You will need to make \package{Homebrew} aware of \ctg'
...
Next, you must configure the \ctg build using the \cli{configure} script.
\begin{enumerate}
    \item If you want to use the \ctg Python interface you must pass the \cli{--enable-python} option
    to \cli{configure}. The default is \cli{--disable-python}.

    \item If you want to use the \ctg \gls{PMC} algorithm you must pass the \cli{--enable-pmc} option
...
If you installed \ctg to a non-standard location (\ie not
...
If you intend to make changes to the \ctg source code you can still use \package{Homebrew}.
...
\end{commandline}
To setup your local for use with \package{Homebrew} you must ``tap'' the \ctg ``formula'' as usual, and then edit it:
\begin{commandline}
brew tap ctg/ctg
brew edit ctg
\end{commandline}
...
Proceed to install the modified \ctg version via \cli{brew install --HEAD ctg}.
To reinstall \ctg after further changes to the source code use
\cli{brew reinstall --HEAD ctg}.\\
