...
To make an independent copy, the \ctg classes provide the \cpp{clone()} method.


\section{Core Classes}
\label{sec:interface:classes}

At the core of the ctg API there are four classes:
\begin{description}[leftmargin=!,labelwidth=.15\textwidth]
    \item[\class{Parameters}]
        which contains the whole set of \class{Parameter} known to ctg
...
\cpp{Parameter}. The default set of parameters is loaded from the \ctg-supplied YAML files. It can be
...
\refsec{extending:observable} for details on how to extend \ctg with the implementation
of a new observable.\\
